...
// Update the postEvents function to accept eventStartDate and eventEndDate
const postEvents = async (req, res) => {
    try {
        const eventData = req.body;

        // Validating the required fields
        const { eventName, eventOrganizerEmailId, eventStartDate, eventEndDate, eventLocation,eventTicketsCount, eventTicketsPrice } = eventData;

        // Calculate eventDuration based on eventStartDate and eventEndDate
        const eventDuration = new Date(eventEndDate) - new Date(eventStartDate);
        ...
        const newEvent = new Events({
            ...
            eventDuration, // Storing eventDuration as number
            eventLocation,
            eventTicketsCount,
            eventTicketsPrice
        });
...
// Update the updateEventById function to calculate eventDuration if eventStartDate or eventEndDate changes
const updateEventById = async (req, res) => {
    try {
        ...
        // If eventStartDate or eventEndDate is being updated, calculate eventDuration
        if (eventData.eventStartDate || eventData.eventEndDate) {
            const eventStartDate = eventData.eventStartDate ? new Date(eventData.eventStartDate) : existingEvent.eventStartDate;
            const eventEndDate = eventData.eventEndDate ? new Date(eventData.eventEndDate) : existingEvent.eventEndDate;
            const eventDuration = eventEndDate - eventStartDate;
            eventData.eventDuration = eventDuration; // Storing eventDuration as number
        }
